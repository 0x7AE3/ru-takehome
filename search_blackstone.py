...
class JobSearch:
    ...
    def _compute_embeddings(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            jobs = conn.execute("""
                SELECT rowid, title, description, location, time_type 
                FROM jobs
            """).fetchall()
    ...
    def search(self, query: str, top_k: int = 5) -> List[Dict]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for job_id, similarity in top_jobs:
                job = conn.execute("""
                    SELECT rowid, title, description, location, human_url, 
                           api_url, posted_date, start_date, time_type
...
if __name__ == "__main__":
    load_dotenv()
    searcher = JobSearch("blackstone.db", recompute_embeddings=("-f" in sys.argv))
    ...
    for i, result in enumerate(results, 1):
        print(f"{i}. {result['title']} ({result['location']})")
        print(f"   Time Type: {result['time_type']}")
        print(f"   Posted Date: {result['posted_date']}")
        print(f"   Start Date: {result['start_date']}")
        print(f"   Similarity Score: {result['similarity']:.3f}")
        print(f"   URL: {result['human_url']}")
    print("" + "="*50)
